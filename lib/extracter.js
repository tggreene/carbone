...
var extracter = {
  ...
  splitMarkers : function (markers) {
    ...
    for (var i = 0; i < markers.length; i++) {
      ...
      var _markerType = markers[i].type;
      ...
      for (var c = 0; c < _marker.length; c++) {
        ...
        if (_char===':' && _isFormatterParenthesis === false) {
          _isFormatter = true;
          // if we enter in a second formatter, keep the previous one. Ex. three nested formatters : formater1:formater2:formater3
          if (_formatterStr !== '') {
            _formatters.push(_formatterStr);
            _formatterStr = '';
          }
        }
      ...
      if (_iterators.length === 0) {
        ...
        // If the marker is in a urlencoded string, automatically add uriencoder
        if (_markerType === 'urlencoded') {
          _formatters.push('urlEncode');
        }
