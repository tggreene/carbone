...
var parser = {
  ...
  isCarboneMarker : function (oneMarker) {
    const _isCarboneMarkerRegexp = /^\{?\s*(?:[cd]\s*[.[:])|\$/;
  ...
  findMarkers : function (xml, callback) {
    ...
    _cleanedXml = _cleanedXml.replace(/(\{|%7B)([^{]+?)(\}|%7D)/g, function (m, brace1, marker, brace2, offset) {
      ...
      var _obj = {
        ...
        type : isURLEncoded ? 'urlencoded' : 'plain'
      };
      _allMarkers.push(_obj);
      _previousMarkerLength += (marker.length + (isURLEncoded ? 6 : 2));  // 2 equals the number of braces '{' or '}'
